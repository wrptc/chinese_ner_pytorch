...
class BiLSTMCRF(nn.Module):
    ...
    def __init__(
        ...
        # self.word_embeddings = BertModel.from_pretrained('bert-base-chinese')
        ...
        #                 num_layers=1, bidirectional=False, batch_first=True, dropout=self.dropout)
        self.hidden2tag = nn.Linear(self.hidden_dim, self.tag_size)
        self.hidden = self.init_hidden()
        self.projection = nn.Sequential(nn.Linear(self.hidden_dim, 64), nn.ReLU(), nn.Linear(64, self.hidden_dim))
    ...
    def __get_lstm_features(self, sentence):
        ...
        # lstm_out, self.hidden = self.gru(embeddings)
